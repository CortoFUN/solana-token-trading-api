  ...
  if (executionKey) {
    return {
      headers: {
        ...
        'x-idempotency-key': String(process.env.CORTO_IDEMPOTENCY_KEY || `lightning-sell-${crypto.randomUUID()}`)
      },
      mode: 'agent'
    };
  }
...
if (!amount) {
  throw new Error('Missing CORTO_LIGHTNING_SELL_AMOUNT');
}
...
const payload = {
  ...
  memo: String(process.env.CORTO_LIGHTNING_MEMO || 'agent starter script lightning sell'),
