...
const TOKEN_TRADE_TAPE_LIMIT = 50;
const AGENT_HISTORY_LIMIT = 8;
const SOL_PRICE_REFRESH_MS = 2 * 60 * 1000;
const PUBLIC_DIR = fileURLToPath(new URL('./public/', import.meta.url));
const DEFAULT_STREAM_LAUNCHPADS = Object.freeze([
  Object.freeze({ id: 'raydium-launchlab', label: 'Raydium' }),
  Object.freeze({ id: 'pump.fun', label: 'Pump' }),
  Object.freeze({ id: 'meteora-dbc', label: 'Meteora' }),
  Object.freeze({ id: 'letsbonk.fun', label: 'LetsBonk' })
]);
...
function getLaunchpadLabel(value) {
  const launchpad = normalizePool(value);

  if (!launchpad) {
    return null;
  }

  if (launchpad === 'raydium-launchlab') {
    return 'Raydium';
  }
  ...
  if (launchpad === 'meteora-dbc') {
    return 'Meteora';
  }

  if (launchpad === 'letsbonk.fun') {
    return 'LetsBonk';
  }
...
function buildStreamLaunchpads(tokenBuilderCapabilities) {
  const catalog = new Map(DEFAULT_STREAM_LAUNCHPADS.map((entry) => [entry.id, entry]));

  const platforms = Array.isArray(tokenBuilderCapabilities?.platforms)
    ? tokenBuilderCapabilities.platforms
    : [];

  platforms.forEach((platform) => {
    const id = normalizePool(platform?.slug || platform?.id);
    if (!id) {
      return;
    }
    ...
    catalog.set(id, {
      id,
      label: getLaunchpadLabel(id) || normalizeOptionalText(platform?.label) || id
    });
  });
...
function getOptionalUserApiKey(request) {
  return normalizeOptionalText(request.get('x-api-key') || request.get('x-corto-user-api-key'));
}

function getOptionalSettingsKey(request) {
  return normalizeOptionalText(request.get('x-settings-key'));
}
...
function deriveTraderAddress(payload) {
  return pickLastNonEmpty(
    payload?.trader,
...
function buildTradeTapeEntry(eventPayload, existingToken = null) {
  const marketCapSol = pickLastFinite(eventPayload?.marketCapSol, existingToken?.marketCapSol);
  const marketCapUsd = marketCapSol !== null && streamState.solPriceUsd !== null
    ? marketCapSol * streamState.solPriceUsd
    : null;

  return {
    mint: normalizeText(eventPayload?.mint),
    signature: normalizeText(eventPayload?.signature),
    txType: normalizeText(eventPayload?.txType) || 'trade',
    trader: deriveTraderAddress(eventPayload),
    amountSol: deriveVolumeSol(eventPayload),
    marketCapSol,
    marketCapUsd,
    timestampMs: deriveTimestampMs(eventPayload)
  };
}

function pushTradeTapeEntry(mint, entry) {
  if (!mint || !entry) {
    return null;
  }

  const existingTape = Array.isArray(streamState.tokenTradeTape.get(mint))
    ? streamState.tokenTradeTape.get(mint)
    : [];

  if (entry.signature && existingTape.some((item) => item.signature === entry.signature)) {
    return null;
  }

  const nextTape = [entry, ...existingTape].slice(0, TOKEN_TRADE_TAPE_LIMIT);
  streamState.tokenTradeTape.set(mint, nextTape);
  return entry;
}
...
function removeStreamToken(mint) {
  ...
  streamState.tokenTradeTape.delete(mint);
...
function handleTradeEvent(eventPayload) {
  ...
  const entry = pushTradeTapeEntry(mint, buildTradeTapeEntry(eventPayload, streamState.tokenCache.get(mint) || existing));
  if (entry) {
    broadcastTradeTape(mint, entry);
  }
}
...
function broadcastTradeTape(mint, entry) {
  if (!mint || !entry) {
    return;
  }
  ...
  streamState.clients.forEach((client) => {
    ...
    if (client.subscribedTradeMint !== mint) {
      return;
    }
...
function readAgentHistoryLimit(value) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    return AGENT_HISTORY_LIMIT;
  }

  return Math.min(50, Math.round(parsed));
}

async function proxyJsonUpstream(response, path, options) {
  try {
    const upstream = await fetchUpstream(path, options);
    return response.status(upstream.status).json(upstream.body);
  } catch (error) {
    return response.status(500).json({
      success: false,
      error: {
        code: 'TRADING_TERMINAL_V1_PROXY_ERROR',
...
function processUpstreamMessage(rawData) {
  ...
  if (typeof rawData === 'string') {
    ...
    rawText = Buffer.from(rawData.buffer, rawData.byteOffset, rawData.byteLength).toString('utf8');
  }
...
app.get('/api/runtime', async (_request, response) => {
  ...
  const streamLaunchpads = buildStreamLaunchpads(tokenBuilderCapabilities.body);
...
app.get('/api/stream/trades/:mint', (request, response) => {
  const mint = normalizeText(request.params.mint);

  if (!mint) {
    return response.status(400).json({
      success: false,
      error: {
        code: 'TRADING_TERMINAL_V1_MINT_REQUIRED',
        message: 'Mint is required.'
      }
    });
  }

  const trades = Array.isArray(streamState.tokenTradeTape.get(mint))
    ? streamState.tokenTradeTape.get(mint)
    : [];
...
app.post('/api/wallet/generate', async (_request, response) => proxyJsonUpstream(response, '/api/v1/wallet/generate', {
  method: 'POST',
  headers: {
...
app.post('/api/verify', async (request, response) => {
  const apiKey = getOptionalUserApiKey(request);

  if (!apiKey) {
    return response.status(400).json(createConfigError('Paste an API key before running verify.'));
  }

  return proxyJsonUpstream(response, '/api/v1/verify', {
    method: 'POST',
    headers: {
      'x-api-key': apiKey
    }
...
app.get('/api/agent-wallet/capabilities', async (_request, response) => proxyJsonUpstream(response, '/api/v1/agent-wallet/capabilities', {
  method: 'GET'
}));

app.post('/api/agent-wallet/generate', async (_request, response) => proxyJsonUpstream(response, '/api/v1/agent-wallet/generate', {
  method: 'POST',
  headers: {
    'content-type': 'application/json'
  },
...
app.get('/api/agent-wallet/profile', async (request, response) => {
  const settingsKey = getOptionalSettingsKey(request);

  if (!settingsKey) {
    return response.status(400).json(createConfigError('Paste a settings key before loading the agent profile.'));
  }

  return proxyJsonUpstream(response, '/api/v1/agent-wallet/profile', {
    method: 'GET',
    headers: {
...
app.get('/api/agent-wallet/balances', async (request, response) => {
  const settingsKey = getOptionalSettingsKey(request);

  if (!settingsKey) {
    return response.status(400).json(createConfigError('Paste a settings key before loading balances.'));
  }

  return proxyJsonUpstream(response, '/api/v1/agent-wallet/balances', {
    method: 'GET',
    headers: {
      'x-settings-key': settingsKey
    }
...
app.get('/api/agent-wallet/stats', async (request, response) => {
  const settingsKey = getOptionalSettingsKey(request);

  if (!settingsKey) {
    return response.status(400).json(createConfigError('Paste a settings key before loading agent stats.'));
  }

  return proxyJsonUpstream(response, '/api/v1/agent-wallet/stats', {
    method: 'GET',
    headers: {
      'x-settings-key': settingsKey
    }
...
app.get('/api/agent-wallet/history', async (request, response) => {
  const settingsKey = getOptionalSettingsKey(request);

  if (!settingsKey) {
    return response.status(400).json(createConfigError('Paste a settings key before loading agent history.'));
  }

  const limit = readAgentHistoryLimit(request.query.limit);

  return proxyJsonUpstream(response, `/api/v1/agent-wallet/history?limit=${limit}`, {
    method: 'GET',
    headers: {
      'x-settings-key': settingsKey
    }
...
app.post('/api/agent-wallet/settings', async (request, response) => {
  const settingsKey = getOptionalSettingsKey(request);

  if (!settingsKey) {
    return response.status(400).json(createConfigError('Paste a settings key before saving agent settings.'));
  }

  return proxyJsonUpstream(response, '/api/v1/agent-wallet/settings', {
    method: 'POST',
    headers: {
      'content-type': 'application/json',
      'x-settings-key': settingsKey
...
frontendStreamServer.on('connection', (socket) => {
  socket.subscribedTradeMint = null;
  ...
  socket.on('message', (rawData) => {
    ...
    if (payload?.type === 'subscribe_trades') {
      socket.subscribedTradeMint = normalizeText(payload?.mint);
      return;
    }

    if (payload?.type === 'unsubscribe_trades') {
      socket.subscribedTradeMint = null;
    }
  });
