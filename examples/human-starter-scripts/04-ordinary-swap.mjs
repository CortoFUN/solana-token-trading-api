...
if (!INPUT_MINT) throw new Error('Missing CORTO_SWAP_INPUT_MINT');
if (!OUTPUT_MINT) throw new Error('Missing CORTO_SWAP_OUTPUT_MINT');
if (!Number.isFinite(AMOUNT) || AMOUNT <= 0) throw new Error('Missing or invalid CORTO_SWAP_AMOUNT');
...
const payload = {
  ...
  memo: String(process.env.CORTO_SWAP_MEMO || 'human starter script swap'),
